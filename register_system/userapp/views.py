...
from userapp.models import Person, Team
from userapp.models import Setting
import register_system.settings

MEMBER_LIMIT = 3

team_number_lx = lambda :len(Team.objects.filter(match_zone='LX'))
team_number_zg = lambda :len(Team.objects.filter(match_zone='ZG'))
team_number_single = lambda :len([x for x in Team.objects.all() if x.is_new_student_team()])
...
def register(request):
    ...
    if request.method == 'POST':
        ...
        with transaction.atomic():
            ...
            team_limit_single = Setting.objects.get(pk=1).single_person_team_limited
            ...
            user = User.objects.create_user(captain_email, captain_email, password)
            team = Team.objects.create(name=team_name, email=captain_email, phone_number=captain_phone, user=user, match_zone=match_zone, team_id=team_id)
